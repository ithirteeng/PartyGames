    ...
    private val sendOriginatingMessageRunnable = Runnable {
        ...
        neighbors.forEach {
            ...
            try {
                ...
                logger(
                    Log.WARN,
                    "$logPrefix : sendOriginatingMessagesRunnable: exception sending to " +
        ...
        if (stationNeighborInetAddr != null &&
            ...
            logger(
                Log.WARN, "$logPrefix : sendOriginatingMessagesRunnable: have not received " +
            ...
            try {
                ...
                logger(
                    Log.ERROR, "$logPrefix : sendOriginatingMessagesRunnable: could not " +
            ...
            logger(
                Log.WARN, "$logPrefix : sendOriginatingMessagesRunnable : could not send " +
    ...
    fun onReceiveOriginatingMessage(
        ...
        if (currentOriginatorMessage == null || isMoreRecentOrBetter) {
            originatorMessages[virtualPacket.header.fromAddr] = VirtualNode.LastOriginatorMessage(
                ...
                deviceName = virtualPacket.header.deviceName
            )
            logger(
                Log.VERBOSE,
                message = {
    ...
    fun lookupNextHopForChainSocket(address: InetAddress, port: Int): ChainSocketNextHop {
        ...
        return when {
            ...
            originatorMessage != null && originatorMessage.hopCount == 1.toByte() -> {
                ChainSocketNextHop(
                    address = originatorMessage.lastHopRealInetAddr,
                    port = port,
                    isFinalDest = true,
                    network = originatorMessage.receivedFromSocket.boundNetwork
                )
            }
            ...
            originatorMessage != null -> {
                ChainSocketNextHop(
                    address = originatorMessage.lastHopRealInetAddr,
                    port = originatorMessage.lastHopRealPort,
                    isFinalDest = false,
                    network = originatorMessage.receivedFromSocket.boundNetwork
                )
            }
